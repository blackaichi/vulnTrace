...
| Still open | 4 — RWF-001, RWF-006, RWF-044, RWF-046 | structural — the same table | NOT all of one kind, and the difference matters: RWF-001 and RWF-006 are precision gaps that degrade to UNKNOWN in both directions, RWF-044 is precision-only by construction, but RWF-046 is a SOUNDNESS finding in BOTH directions (a function-local require binding at file scope, so two same-named locals collapse). Reading this row as 'open precision debt' is the misreading to avoid. | 'Open' is a status word in a table, not a scheduled task. See `docs/OPEN-DEBTS.md`. |
