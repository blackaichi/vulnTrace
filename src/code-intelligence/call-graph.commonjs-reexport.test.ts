...
describe("buildCallGraph: the CommonJS re-export chase never guesses (RWF-004a negative controls)", () => {
  ...
  it("does NOT cross a package boundary, even for an identically-named export (RWF-004b stays unimplemented)", async () => {
    const root = tempProject();
    writePackage(
      root,
      `exports.vulnerable = require("other-lib").vulnerable;\n`,
    );
    write(
      root,
      "node_modules/other-lib/package.json",
      JSON.stringify({ name: "other-lib", version: "1.0.0", main: "index.js" }),
    );
    write(
      root,
    ...
    // And specifically: nothing bound to the other package's function.
    const otherFn = findNode(
      graph,
      ...
      expect(
        graph.edges.some(
          (e) =>
            ...
            e.resolution.target === otherFn.id,
        ),
      ).toBe(false);
    }
  });

  it("does NOT escape its own package via a relative `../` specifier", async () => {
    const root = tempProject();
    writePackage(
      root,
      `exports.vulnerable = require("../sibling-lib/impl.js").vulnerable;\n`,
    );
    write(
      root,
      "node_modules/sibling-lib/package.json",
      JSON.stringify({ name: "sibling-lib", version: "1.0.0" }),
    );
    write(
      root,
