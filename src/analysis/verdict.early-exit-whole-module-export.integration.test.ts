...
/**
 * RWF-015's permanent end-to-end regression (see
 * fixtures/commonjs-early-exit-whole-module-export/README.md).
 *
 * The fixture's `fixture-lib/index.js` exports the vulnerable branch and
 * then `return`s out of module evaluation; `thrower.js` does the same with
 * an uncaught `throw`. In BOTH files the last `module.exports` write is a
 * syntactically unconditional top-level statement -- which is precisely
 * what RWF-014's authority rule accepted. The module's whole exported
...
const FIXTURE = "commonjs-early-exit-whole-module-export";
const RETURN_ENTRYPOINT = "src/index.cjs";
const THROW_ENTRYPOINT = "src/thrower.cjs";
...
describe("RWF-015 fixture: an early top-level exit bypassing the final whole-module export (fixtures/commonjs-early-exit-whole-module-export)", () => {
  for (const [label, entrypoint, file] of [
    ["early RETURN", RETURN_ENTRYPOINT, "index.js"],
    ["early THROW", THROW_ENTRYPOINT, "thrower.js"],
  ] as const) {
    describe(label, () => {
      ...
      it("attributes no definitive whole-module target to either branch", async () => {
        ...
        const branchNodes = graph.nodes.filter(
          (n) =>
            ...
            n.module.endsWith(path.join("fixture-lib", file)),
        );
