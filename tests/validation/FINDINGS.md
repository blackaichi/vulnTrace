...
deliberately, in the category that cannot overstate closure. **Every
`## <ID>` finding section needs a row here, and every row a section.**
A mismatch either way fails the generator, naming the ID.
...
| RWF-013 | any CommonJS file that reassigns an exported local (real shapes in `es-define-property`, `gopd`) | An export whose value is an identifier the file itself REASSIGNS fell through to a same-file name search, which lands on the binding's STALE initializer — an anonymous function expression is indexed under the name of the variable it was assigned to, so the stale node matches the export name exactly | **Soundness** — reproduced end-to-end as a false `NOT_AFFECTED` carrying a complete Family C unreachability proof over a function the module does not export | **Fixed** (variable bindings; declaration form: RWF-013b) — see below |
...
| RWF-023 | any file invoking a vulnerable target from a COMPUTED CLASS-ELEMENT KEY (`class C { [key()]() {} }`) | A computed class-element key is evaluated at class definition, but the call in it was omitted from module-load reachability | **Soundness** — P0: false `NOT_AFFECTED` with a complete Family C proof (the section's Severity line) | **Fixed** (RWF-023) — its listed limitations are other findings or refusals — see below |
| RWF-024 | any CommonJS file with a throwing local call in an OBJECT LITERAL's COMPUTED KEY (`const o = { [bail()]: 1 }`) | The throwing key ends module evaluation, but later CommonJS exports kept their authority | **Soundness** — P0: false `NOT_AFFECTED` with a complete Family C proof (the section's Severity line) | **Fixed** (RWF-024) — its property-value candidate was since fixed by RWF-026 — see below |
| RWF-025 | any CommonJS file whose destructuring target has a COMPUTED KEY | A destructuring computed key poisoned module-wide reassignment provenance, which could silently disable merged abruptness fixes for a whole file | **Soundness** — P0: false `NOT_AFFECTED` with a complete Family C proof (the section's Severity line) | **Fixed** (RWF-025) — see below |
| RWF-025b | any CommonJS file whose re-export provenance crosses a broad assignment-target traversal | The same broad assignment-target traversal as RWF-025, in CommonJS re-export provenance | **Precision only** — P2: lost re-export attribution, `UNKNOWN`, never a false `NOT_AFFECTED` (the section's Severity line) | **Fixed** (RWF-025b) — see below |
| RWF-026 | any CommonJS file with a definitely-abrupt call in a NECESSARILY EVALUATED expression position (`foo(bail())`, `1 + bail()`) | Each earlier fix recognised a definitely-abrupt call in one named syntactic slot only | **Soundness** — P0: false `NOT_AFFECTED` with a complete Family C proof (the section's Severity line) | **Fixed** (RWF-026) — the section also records a separate, inherited MAY-execute conditional-operand family (`flag && bail()`), which has no register row of its own — see below |
| RWF-027 | any CommonJS file whose class `extends` heritage calls a function every analyzable ending of which prevents the class definition from completing | A heritage callee that throws on one path and returns an invalid value on another was not recognised as preventing the class definition | **Soundness** — P0: false `NOT_AFFECTED` with a complete Family C proof (the section's Severity line) | **Fixed** (RWF-027) — see below |
| RWF-028 | any CommonJS file invoking an already-proven-fatal callee through an alias, member, transitive or `new` callee | The callee was proven fatal but its PROVENANCE was unresolved, so the invocation did not end module evaluation | **Soundness** — P0: false `NOT_AFFECTED` with a complete Family C proof (the section's Severity line) | **Fixed** (RWF-028) — see below |
| RWF-032 | monorepos: a workspace's own local packages (P1-A4) | Local packages had no identity path of their own, and Site B answered advisories from project-root resolution with no instance gating | **Soundness / target identity**, both directions — a removed false `NOT_AFFECTED` and removed false `AFFECTED`s | **Fixed** (P1-A4, corrected twice: RWF-032 CORRECTION and CORRECTION 2 record the code defects fixed, wrong-package matrix at 0 violations) — see below |
| RWF-033 | any advisory with several installed instances (P1-A5) | Instances shared one identity in the report, and enumeration could not see an instance the lockfile had no version for | **Explainability**, plus new coverage; no corpus verdict moved | **Fixed** (P1-A5; RWF-033 REMEDIATION: "both are fixed here") — see below |
| RWF-034 | workspaces and installed `node_modules` (Foundation F1) | Workspace incompleteness was invisible to machines, and a divergent `node_modules` could not contradict the lockfile | **Uncertainty representation**; in the version case, wrong verdicts in both directions (the section) | **Fixed** (Foundation F1; "closed here", and RWF-040: "Foundation (F1–F7) is COMPLETE") — see below |
| RWF-035 | the negative-proof preconditions (Foundation F2) | An absent module-load closure was read as a satisfied guard, and an unclassified dynamic reason was read as safe | **Soundness preconditions** — a missing guard state could fail OPEN | **Fixed** (Foundation F2; RWF-040: "Foundation (F1–F7) is COMPLETE") — see below |
| RWF-036 | the uncertainty model (Foundation F3) | Uncertainty was prose, and a candidate that produced no finding said nothing | **Explainability** — no verdict moved | **Fixed** (Foundation F3; RWF-040: "Foundation (F1–F7) is COMPLETE") — see below |
| RWF-037 | every negative-proof prerequisite (Foundation F4) | The guards existed; no stated invariant or harness proved each one load-bearing | **Soundness assurance** — test-only, no production behaviour | **Fixed** (Foundation F4; RWF-040: "Foundation (F1–F7) is COMPLETE") — see below |
| RWF-038 | the analyzer's filesystem access (Foundation F5) | The analyzer re-asked the filesystem the same question thousands of times | **Performance** — never proof authority | **Fixed** (Foundation F5; RWF-040: "Foundation (F1–F7) is COMPLETE") — see below |
| RWF-039 | the gates and the metadata validator (Foundation F6) | The invariants had no written owners, and the metadata validator checked something else | **Assurance** — gate ownership and history validation | **Fixed** (Foundation F6; RWF-040: "Foundation (F1–F7) is COMPLETE") — see below |
| RWF-040 | the project's documentation (Foundation F7) | The invariants were real, owned and tested, but undocumented except in commit history | **Explainability** — zero production files changed | **Fixed** (Foundation F7; "Foundation (F1–F7) is COMPLETE") — see below |
...
## RWF-046b — Pointer: the pre-existing destructured-require shapes closed by RWF-046a

Appended by task `scorecard-status-classifier`, so that every row of the
status table resolves to a section. It adds no finding, and it changes
nothing that RWF-046a records.

RWF-046b's status row already existed. Its defect is described, measured
and closed inside **RWF-046a**, under **"The base was not clean — five
further defects, each closed here"**. Those are the destructured-require
shapes that were **pre-existing on `main`**: computed, numeric and
string-literal property keys, a defaulted element, and reassignment of a
destructured require binding. The scoped re-audit measured them on
`b9bb81b`. RWF-046a's shape boundary closed them, and they were recorded
