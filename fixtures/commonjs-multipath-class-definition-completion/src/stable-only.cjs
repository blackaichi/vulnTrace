...
// at all: it never requires the multi-path modules. The Family C positive
// control is scanned from here so that it proves what it claims to -- that
// an UNCONDITIONAL whole-module export with nothing bypassing it above is
// still attributable and still supports a negative proof -- rather than
// riding on whatever uncertainty the RWF-027 modules contribute.
